...
class FullDataDownloader:
    ...
    def extract_resources(self):
        for result in self.object_results:
            ...
            for resource in safe_get(result, ["resources"], []):
                if (
                    self.potential_metadata_resource(resource)
                    and safe_get(resource, ["format"], "Sin formato").lower()
                    in self.extension_white_list
                ):
    ...
    def download_resources(self):
        ...
        for resource_id, resource_info in self.resources.items():
            ...
            for table_resource_id, table_resource in (
                resource_info["table_resources"].copy().items()
            ):
                csv_file_name = os.path.join(
                    package_folder, f"table_{table_resource_id}.csv"
                )
                ...
                if (
                    ...
                    print(
                        f"Skipping download of {table_resource_id}. Enough table files in the directory."
                    )
                ...
                if table_resource["size"] and table_resource["size"] > 42000000:
                    print(
                        f"Skipping download of {table_resource_id}. File size exceeds 42MB."
                    )
                    resource_info["table_resources"].pop(table_resource_id)
                    continue
            ...
            for potential_metadata_info_id, potential_metadata_info in (
                resource_info["potential_metadata_resources"].copy().items()
            ):
                file_name = os.path.join(
                    package_folder,
                    f"potential_metadata_{potential_metadata_info_id}.{potential_metadata_info['format'].lower()}",
                )
                ...
                if (
                    len(
                        [
                            f
                            for f in os.listdir(package_folder)
                            if f.startswith("potential_metadata")
                        ]
                    )
                    >= 3
                ):
                    print(
                        f"Skipping download of {potential_metadata_info_id}. Enough table files in the directory."
                    )
                    resource_info["potential_metadata_resources"].pop(
                        potential_metadata_info_id
                    )
                    continue

                # Limite de tamaño de archivo
                if (
                    potential_metadata_info["size"]
                    and potential_metadata_info["size"] > 42000000
                ):
                    print(
                        f"Skipping download of {potential_metadata_info_id}. File size exceeds 42MB."
                    )
                    continue
        ...
        print(
            "Cantidad total de tablas descartados por error al descargar:",
            total_tables_error,
        )
        print("Cantidad total de metadata procesadas:", total_metadata_processed)
        print(
            "Cantidad total de metadata descartadas por error al descargar:",
            total_metadata_error,
        )
