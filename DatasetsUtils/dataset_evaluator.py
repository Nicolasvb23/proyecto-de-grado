...
class DatasetEvaluator:
    ...
    def prepare(self):
        ...
        for package in packages[:third]:
            ...
            additional_info = self.load_additional_info(
                os.path.join(self.directory, package)
            )

            additional_info["notes"] = ""
            write_file(
                os.path.join(self.directory, package, "additional_info.json"),
                additional_info,
                "json",
                "utf-8",
            )
        ...
        for package in packages[third : 2 * third]:
            ...
            if not metadata_file:
                continue
            os.remove(
                os.path.join(self.directory, package, f"metadata_{metadata_file}.json")
            )
