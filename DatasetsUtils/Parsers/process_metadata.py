...
class MetadataProcessor:
    ...
    def process_directory(self, root, dir_name):
        ...
        for filename in os.listdir(dir_path):
            ...
            extension, content, encoding = (
                self.recognize_and_process_potential_metadata(file_path)
            )
            ...
            if filename.startswith("potential_metadata_"):
                ...
                if extension == "json":
                    ...
                    additional_info["metadata_resources"][file_id] = additional_info[
                    ...
                    file_output = os.path.join(
                        output_dir, f"potential_metadata_resources{file_id}.txt"
                    )
                    write_file(file_output, content, "txt", encoding)
                    additional_info["potential_metadata_resources"][file_id][
                        "format"
                    ] = "txt"
    ...
    def process_all(self):
        ...
        print(
            "Cantidad total de archivos de metadata procesados:",
            self.total_potential_metadata_processed,
        )
        print(
            "Cantidad total de archivos de metadata descartados por error de extensión errónea:",
            self.total_error_extension,
        )
        print(
            "Cantidad total de archivos de metadata descartados por error de encoding:",
            self.total_error_encoding,
        )
