...
"""
    ...
    def description_with_metadata_prompt(
        self, table, table_id, additional_info, metadata, few_shots_prompt_data
    ):
...
"""
    ...
    def generate_description_with_metadata(
        self, table, table_id, metadata, additional_info, few_shots_prompt_with_metadata
    ):
        # Tokenize the prompt
        inputs = self.tokenizer(
            self.description_with_metadata_prompt(
                table,
                table_id,
                additional_info,
                metadata,
                few_shots_prompt_with_metadata,
            ),
            return_tensors="pt",
        ).to(self.device)
